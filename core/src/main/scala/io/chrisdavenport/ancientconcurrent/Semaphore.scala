...
import io.chrisdavenport.ancientconcurrent.Semaphore.TransformedSemaphore
...
object Semaphore {
  ...
  abstract private class AbstractSemaphore[F[_]](state: Ref[F, State[F]])(implicit F: Async[F]) extends Semaphore[F] {
    ...
    def acquireN(n: Long): F[Unit] = acquireNInternal(n).flatMap{ case (g, _) => g }
  ...
  final private[ancientconcurrent] class TransformedSemaphore[F[_], G[_]](
